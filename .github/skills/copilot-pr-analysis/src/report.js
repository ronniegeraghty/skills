 ...
 */
async function generateBarChart(labels, data, label, color = "rgb(54, 162, 235)") {
 ...
 * 
 * @param {string[]} labels - X-axis labels
 * @param {number[]} mergedData - Merged PR data
 * @param {number[]} abandonedData - Abandoned PR data
 * @returns {Promise<string>} Chart image URL
 */
async function generateComparisonChart(labels, mergedData, abandonedData) {
  const chart = new QuickChart();
  chart.setConfig({
    type: "bar",
    data: {
 ...
 */
async function generateOverview(analysis) {
 ...
 * 
 * @param {object[]} insights - Array of insight objects
  ...
  for (const insight of insights) {
    ...
    if (insight.type === "success-factor" || insight.type === "failure-pattern") {
      ...
      for (const item of insight.items) {
        const rate = insight.type === "success-factor" 
      ...
      for (const item of insight.items) {
        ...
        md += `| ${item.repo} | ${item.successRate}% | ${emoji} ${item.label} |\n`;
      }
      md += "\n";
    }
  }
 ...
 * @returns {Promise<string>} Markdown content
 */
async function generateResourceSection(resourceAnalysis) {
...
`;
  ...
  const topByUsage = [...resources].sort((a, b) => b.totalCount - a.totalCount).slice(0, 15);
  
  for (const r of topByUsage) {
    const scoreEmoji = r.successScore > 0 ? "🟢" : r.successScore < 0 ? "🔴" : "⚪";
 ...
 * @returns {Promise<string>} Markdown content
 */
async function generateToolSection(toolAnalysis) {
...
`;
  ...
  for (const t of sortedTools.slice(0, 15)) {
    const scoreEmoji = t.successScore > 0 ? "🟢" : t.successScore < 0 ? "🔴" : "⚪";
 ...
 * @returns {Promise<string>} Markdown content
 */
async function generateRepoSection(repoAnalysis) {
...
function generateRecommendations(analysis) {
  ...
  if (successFactors && successFactors.items.length > 0) {
    recommendations.push({
      priority: "High",
      recommendation: `Ensure all repositories have the following resources: ${successFactors.items.slice(0, 3).map(i => `\`${i.name}\``).join(", ")}`,
      rationale: "These resources are associated with higher PR success rates."
    });
  }
  ...
  if (failurePatterns && failurePatterns.items.length > 0) {
    recommendations.push({
      priority: "High",
      recommendation: `Review and improve the content of: ${failurePatterns.items.slice(0, 3).map(i => `\`${i.name}\``).join(", ")}`,
      rationale: "These resources appear more often in abandoned PRs, suggesting their content may be misleading or insufficient."
    });
  }
...
async function main() {
  ...
  let analysis;
  try {
    analysis = readOutput(OUTPUT_FILES.ANALYSIS);
  } catch (error) {
