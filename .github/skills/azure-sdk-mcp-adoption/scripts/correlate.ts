...
interface McpUsage {
  ...
  resolvedFromTypespec?: boolean;
  matchedTypespecPath?: string;
}
 ...
 */
function normalizeTypespecPath(path: string): string {
  if (!path) return "";
  return path
    .toLowerCase()
    .replace(/\\/g, "/")
 ...
 */
function typespecPathsMatch(path1: string, path2: string): boolean {
...
interface LoadedData {
  telemetryData: TelemetryOutput;
  releasesData: ReleasesOutput;
}
 ...
 */
function loadDataFiles(outputDir: string): LoadedData {
  let telemetryData: TelemetryOutput | null = null;
  let releasesData: ReleasesOutput | null = null;
  ...
  if (!telemetryData || !releasesData) {
    ...
    if (found?.files) {
      if (!telemetryData && found.files["telemetry.json"]) {
        telemetryData = JSON.parse(
          readFileSync(found.files["telemetry.json"], "utf-8")
        );
        console.log(`  Loaded telemetry from: ${found.files["telemetry.json"]}`);
      }
      if (!releasesData && found.files["releases.json"]) {
        releasesData = JSON.parse(
          readFileSync(found.files["releases.json"], "utf-8")
        );
      ...
      if (!telemetryData) {
        telemetryData = JSON.parse(
          readFileSync(join(found.dir, "telemetry.json"), "utf-8")
        );
        console.log(`  Loaded telemetry from: ${found.dir}`);
      }
      if (!releasesData) {
        releasesData = JSON.parse(
          readFileSync(join(found.dir, "releases.json"), "utf-8")
        );
  ...
  if (!telemetryData || !releasesData) {
    throw new Error(
      "Required data files not found. Run fetch-telemetry and fetch-releases first."
    );
  }
...
interface LookupMaps {
  packagesWithMcp: Map<string, PackageSummaryItem>;
  typespecPathsWithMcp: Map<string, TypespecSummaryItem>;
}

/**
 * Build lookup maps for MCP usage by package name and TypeSpec path
 */
function buildLookupMaps(telemetryData: TelemetryOutput): LookupMaps {
  // Build package lookup
  const packagesWithMcp = new Map<string, PackageSummaryItem>();
  ...
  const typespecPathsWithMcp = new Map<string, TypespecSummaryItem>();
...
function correlateRelease(
  ...
  let mcpUsage: McpUsage | null =
    packagesWithMcp.get(normalized) ||
    packagesWithMcp.get(release.packageName.toLowerCase()) ||
    null;
  let matchType: "package" | "typespec" | null = mcpUsage ? "package" : null;
 ...
 */
function calculateGroupedStats<K extends string>(
  releases: CorrelatedRelease[],
  field: keyof CorrelatedRelease
): Array<{ [key in K]: string } & { total: number; withMcp: number; adoptionRate: number }> {
  ...
  return Object.entries(byGroup)
    .map(([key, stats]) => ({
      ...
      adoptionRate:
        stats.total > 0
          ? Math.round((stats.withMcp / stats.total) * 10000) / 100
          : 0,
    }))
    .sort((a, b) => b.total - a.total) as Array<
    { [key in K]: string } & { total: number; withMcp: number; adoptionRate: number }
  >;
}
...
function printSummary(
  correlatedReleases: CorrelatedRelease[],
  languageStats: LanguageStat[],
  versionTypeStats: VersionTypeStat[],
  planeStats: PlaneStat[],
  directMatches: number,
  typespecMatches: number
): void {
  console.log("\n=== Correlation Summary ===");
  ...
  for (const lang of languageStats) {
    console.log(
      `  ${lang.language}: ${lang.withMcp}/${lang.total} (${lang.adoptionRate}%)`
    );
  }
  ...
  for (const vt of versionTypeStats) {
    console.log(
      `  ${vt.versionType}: ${vt.withMcp}/${vt.total} (${vt.adoptionRate}%)`
    );
  }
  ...
  if (withMcp.length > 0) {
    ...
    withMcp.slice(0, 10).forEach((r, i) => {
      ...
      console.log(
        `  ${i + 1}. ${r.packageName} v${r.version} (${r.language}, ${r.versionType}) - ${r.mcpCallCount} calls${matchInfo}`
      );
    });
...
async function main(): Promise<void> {
  ...
  console.log(
    `Telemetry period: ${telemetryData.metadata.startDate} to ${telemetryData.metadata.endDate}`
  );
  ...
  console.log(
    `TypeSpec paths with MCP usage (pending correlation): ${typespecPathsWithMcp.size}`
  );
  console.log(
    `TypeSpec to package mappings from releases: ${Object.keys(releasesData.typespecMapping || {}).length}`
  );
  ...
  const correlatedReleases = releasesData.releases.map((release) => {
    const correlated = correlateRelease(
      release,
      packagesWithMcp,
      typespecPathsWithMcp
    );
  ...
  const adoptionRate =
    totalReleases > 0
      ? Math.round((releasesWithMcp / totalReleases) * 10000) / 100
      : 0;

  const languageStats = calculateGroupedStats<"language">(
    correlatedReleases,
    "language"
  ) as LanguageStat[];
  const versionTypeStats = calculateGroupedStats<"versionType">(
    correlatedReleases,
    "versionType"
  ) as VersionTypeStat[];
  const planeStats = calculateGroupedStats<"plane">(
    correlatedReleases,
    "plane"
  ) as PlaneStat[];
