...
import { readOutput, writeOutput, extractMatches } from "./utils.ts";
import type {
  PrsOutput,
  ClassifiedPr,
  SessionData,
 ...
 */
function fetchSessionLog(repo: string, prNumber: number): string | null {
  try {
    const result = spawnSync(
      "gh",
      ["agent-task", "view", "--repo", repo, String(prNumber), "--log"],
      {
        encoding: "utf8",
    ...
    console.warn(
      `Warning: Failed to fetch session for ${repo}#${prNumber}: ${(error as Error).message}`
    );
    return null;
  }
}
 ...
 */
function extractResources(log: string): string[] {
 ...
 */
function extractMcpTools(log: string): string[] {
 ...
 */
function extractMetrics(log: string): SessionMetrics {
  ...
  const fileWrites = (
    log.match(/Writing file:|Creating file:|Updating file:/gi) || []
  ).length;
 ...
 */
function parseSessionLog(log: string | null): SessionData {
  if (!log) {
    return {
      hasLog: false,
      resources: [],
...
async function main(): Promise<void> {
  ...
  let prsData: PrsOutput;
  try {
    prsData = readOutput<PrsOutput>(OUTPUT_FILES.PRS);
  } catch {
  ...
  for (let i = 0; i < prsToAnalyze.length; i++) {
    ...
    console.log(
      `[${i + 1}/${prsToAnalyze.length}] Fetching ${pr.repo}#${pr.number}...`
    );
    ...
    if (sessionData.hasLog) {
      successCount++;
      console.log(
        `  Found ${sessionData.resources.length} resources, ${sessionData.mcpTools.length} MCP tools`
      );
    } else {
      failCount++;
  ...
  writeOutput(OUTPUT_FILES.SESSIONS, output);
